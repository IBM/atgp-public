...
from functions.styblinski_tang import target, feature_space_range
from functions.goldstein_price import source
...
for i in range(number_datasets):
    ...
    # at_y = np.append(source_data[i, x_dims, :], target(samples, mean=0))
...
np.savez('parallel_test_dataset_different_functions', source_data, target_data, test_data, shifts)
